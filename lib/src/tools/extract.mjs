...
function extractTokensInBrowser() {
  ...
  // family -> Set<numeric weight>. Captures observed (family, weight) pairs
  // so the downstream `fonts sync` command knows exactly which CSS2 specs to
  // request from Google Fonts. Separate from `m.family`/`m.weight` which are
  // tallied independently for the DESIGN.md summary.
  const familyWeights = new Map();
  const bump = (map, key) => { if (key) map.set(key, (map.get(key) || 0) + 1); };
  const bumpPair = (fam, w) => {
    if (!fam) return;
    const n = typeof w === 'number' ? w : parseInt(w, 10);
    if (!Number.isFinite(n)) return;
    if (!familyWeights.has(fam)) familyWeights.set(fam, new Set());
    familyWeights.get(fam).add(n);
  };
  ...
  const pairs = [...familyWeights.entries()].map(([family, set]) => ({
    family,
...
// ============================================================================
//
// Emits `_fonts.json` per state so `print_widget fonts sync` can download the
// exact families + weights needed to render Flutter widgets faithfully.
//
// Merges three sources:
//   1. Observed (family, weight) pairs from the DOM (`tokens.fontPairs`).
//   2. Forced Google Fonts specs from `config.forceFonts` (the page may
//      declare `font-family: Inter` without importing it — forceFonts closes
//      that gap for screenshots, and we want the same pairs downloaded into
//      the Flutter project).
//   3. Family frequency from `tokens.typography.families` (as a fallback
//      when no weight pair was observed for a family).
//
// Output shape:
//   {
//     $version: "1.0",
//     source: { url, state, extractor },
//     families: [
//       { family, weights: [400, 500, ...], sources: ["detected"|"forced"] }
//     ],
//     googleFontsCssUrl: "https://fonts.googleapis.com/css2?family=...",
//     forceFontSpecs: ["Inter:wght@400;500;600;700"]
//   }
//
// System fonts (sans-serif, serif, monospace, system-ui, ui-sans-serif, …)
// are filtered — they aren't downloadable and aren't what we want in-project.
...
function parseForceFontSpec(spec) {
  // "Inter:wght@400;500;600;700" → { family: "Inter", weights: [400, 500, 600, 700] }
  // "Roboto:ital,wght@0,400;1,400" → weights preserved as raw strings
  const m = /^([^:]+)(?::(.+))?$/.exec(spec);
  if (!m) return null;
  const family = m[1].replace(/\+/g, ' ').trim();
  const axes = m[2] || '';
  const weights = new Set();
  const wghtM = /wght@([0-9;,]+)/.exec(axes);
  if (wghtM) {
    for (const tok of wghtM[1].split(/[,;]/)) {
      const n = parseInt(tok.trim(), 10);
      if (Number.isFinite(n)) weights.add(n);
    }
  }
  if (weights.size === 0) weights.add(400);
  return { family, weights: [...weights].sort((a, b) => a - b) };
}

function buildFontsReport({ tokens, config, state }) {
  const merged = new Map(); // family -> { weights: Set, sources: Set }

  const addPair = (family, weight, source) => {
    if (isSystemFont(family)) return;
    if (!merged.has(family)) {
      merged.set(family, { weights: new Set(), sources: new Set() });
    }
    const entry = merged.get(family);
    entry.weights.add(weight);
    entry.sources.add(source);
  };

  for (const { family, weights } of tokens.fontPairs || []) {
    for (const w of weights) addPair(family, w, 'detected');
  }

  // Fallback: family seen but no weight pair bumped (e.g. weight parsing
  // returned a non-numeric string). Treat as 400.
  for (const [family] of tokens.typography?.families || []) {
    if (!isSystemFont(family) && !merged.has(family)) {
      addPair(family, 400, 'detected');
    }
  }

  const forceSpecs = config.forceFonts || [];
  for (const spec of forceSpecs) {
    const parsed = parseForceFontSpec(spec);
    if (!parsed) continue;
    for (const w of parsed.weights) addPair(parsed.family, w, 'forced');
  }

  const families = [...merged.entries()].map(([family, info]) => ({
    family,
    weights: [...info.weights].sort((a, b) => a - b),
    sources: [...info.sources].sort(),
  })).sort((a, b) => a.family.localeCompare(b.family));

  // Build a single CSS2 URL that covers every detected family + weight.
  // Consumers can hit this URL directly to fetch TTFs with an old User-Agent.
  const parts = families.map((f) => {
    const fam = f.family.replace(/ /g, '+');
    const wghts = f.weights.join(';');
    return `family=${encodeURIComponent(fam).replace(/%2B/g, '+')}:wght@${wghts}`;
  });
  const googleFontsCssUrl = parts.length > 0
...
async function captureState(page, state, index) {
  ...
  // Fonts report — consumed by `print_widget fonts sync` to download the
  // exact (family, weight) pairs needed in the Flutter project. Keeps the
  // compile pipeline honest: browser uses Inter 400/500/700, Flutter gets
  // Inter 400/500/700, no silent fallback to system sans.
  const fontsReport = buildFontsReport({ tokens, config, state });
  await fs.writeFile(
    path.join(dir, '_fonts.json'),
    JSON.stringify(fontsReport, null, 2),
  );
